...
**このプロジェクトではタッチUIを持たないBambulabP1Sのみがターゲットです。**

P1S Version 1.7以前専用にチューニングをしています。BambuHandyからダウングレードが必要となります。
...
X1C、X1EおよびP2S / X2D / H2S / A1シリーズなどの**シングルノズル機**についても、プロトコル的には同等のため**動作可能と思われますが、積極的なサポートはしません。**
これらの機種は標準のタッチパネルを持っているため、追加のメリットがあまりないため、メインターゲットとしていません。
** P2S / X2D / H2D / H2C についてはBambulabの制限のためLANモード専用になります。**
...
| ステータスLED NexpixelLED | **Yes**                                             | Yes                                                 | **Yes**                                             | **Yes**                                             | **Yes**                                                       | **Yes**                                             |
...
1. **アップデートファームウェアファイルのダウンロード**:
  - 公式xptouchサイトにアクセスして、ファームウェアアップデートファイルを入手してください。
  - 対象機種に応じて、以下の履歴ページから `firmware.bin` を取得してください。

   | 主なボード | ダウンロード履歴ページ |
   | --- | --- |
   | generic ESP32 CYD | https://tac-lab.tech/xptouch-bin/2.8/history.php |
   | ES3C28P | https://tac-lab.tech/xptouch-bin/s3_2.8/history.php |
   | JC3248W535 | https://tac-lab.tech/xptouch-bin/s3_3.5/history.php |
   | JC8048W550 | https://tac-lab.tech/xptouch-bin/5.0/history.php |


2. **ファームウェアファイルをSDカードのルートにコピー**:
  - SDカードをコンピューターのカードリーダーに挿入してください。
  - ダウンロードしたファームウェアアップデートファイルをSDカードのルートディレクトリにコピーしてください。ファイル名は`firmware.bin`とし、サブディレクトリには配置しないでください。

3. **xptouchスクリーンの再起動**:
  - ファームウェアを含むSDカードを、xptouchスクリーンに挿入し再起動してください。

4. **ファームウェアアップデートの適用**:
  - タッチスクリーンの再起動すると、SDカード上のファームウェアアップデートファイルの自動的に検出します。
  - タッチスクリーンはSDに保存されたfirmware.binを適用します。(ダウングレードも可能です。)
...
6.**スクリーンの初期化失敗**
　JC8048W055の場合タッチスクリーンの仕様違いや個体差などで初期化に失敗した場合、XPTouchの画面が表示されず画面の色が数秒おきに赤・青・黄色と順に変わるような状態となることがあります。
　― この問題に対処するには別途記載している、lcd.jsonで調整してください
---

## 5インチ LCD Bus 設定

**5インチ用ファームウェア（JC8048W550）のみの仕様**です。
RGB パネル / Bus_RGB の**個体差調整**が行えます。

- SD ルートに `**lcd.json`** を置くと、起動時に検出し、設定反映のため**再起動**します。
- **Chrome 拡張の「Download filaments ZIP」** のresourceフォルダに下記サンプル設定があります。
`**/lcd_default.json`**（工場に近い設定）
`**/lcd_disable.json`**（細かい調整だけリセット、画面の向きは維持）
