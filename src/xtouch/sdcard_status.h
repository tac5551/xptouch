...
/* SD カードの存在チェックは cardType() が低レイヤ初期化を引き起こしやすいので、
 * 描画/タイマーのたびに呼ばないように「キャッシュして間引く」ためのユーティリティ。
 *
 * - 初回: xptouch_sdcard_setup() 側で mark_present(true) される
 * - SD 抜去後: 呼び出し元が間引き間隔に従って再チェックし、false に落とす
 */
static bool s_sd_present_cached = false;
static uint32_t s_sd_last_check_ms = 0;

static inline void xptouch_sdcard_mark_present(bool present)
{
    s_sd_present_cached = present;
    s_sd_last_check_ms = millis();
}

static inline bool xptouch_sdcard_is_present_cached(uint32_t interval_ms = 1000)
{
    uint32_t now = millis();
