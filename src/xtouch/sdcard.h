...
#define XPTOUCH_SD_SCK 38
#endif
#ifndef XPTOUCH_SD_CMD
#define XPTOUCH_SD_CMD 40
#endif
#ifndef XPTOUCH_SD_D0
#define XPTOUCH_SD_D0 39
#endif
...
#if defined(__XPTOUCH_SCREEN_28__) || defined(__XPTOUCH_SCREEN_S3_050__)

static inline fs::FS &xptouch_sdcard_fs()
{
    return SD;
}

static inline bool xptouch_sdcard_exists(const char *path)
{
    return SD.exists(path);
}

static inline File xptouch_sdcard_open(const char *path, const char *mode = "r")
{
    return SD.open(path, mode);
}

static inline bool xptouch_sdcard_remove(const char *path)
{
    return SD.remove(path);
}

static inline uint8_t xptouch_sdcard_card_type()
{
    return SD.cardType();
}
...
static inline bool xptouch_sdcard_ensure_parent_dir(const char *filePath)
{
    if (!filePath)
        return false;
    ...
    dirbuf[n] = '\0';
    if (xptouch_filesystem_mkdir(xptouch_sdcard_fs(), dirbuf))
        return true;
...
bool xptouch_sdcard_setup(int8_t sd_cs_pin, bool mode1bit = true)
{
    (void)mode1bit;
    ...
    SPI.begin(XPTOUCH_SD_SPI_SCK, XPTOUCH_SD_SPI_MISO, XPTOUCH_SD_SPI_MOSI, cs);
    ...
    xptouch_filesystem_mkdir(xptouch_sdcard_fs(), xptouch_paths_root);
    xptouch_filesystem_mkdir(xptouch_sdcard_fs(), "/tmp");

    xptouch_sdcard_mark_present(true);
    return true;
}
...
bool xptouch_sdcard_setup(int8_t sd_cs_pin, bool mode1bit = true)
{
    ...
    int pin_d3 = XPTOUCH_SD_D3;
    if (!mode1bit && sd_cs_pin >= 0)
        pin_d3 = (int)sd_cs_pin;
    SD_MMC.setPins(XPTOUCH_SD_SCK,XPTOUCH_SD_CMD,XPTOUCH_SD_D0,XPTOUCH_SD_D1,XPTOUCH_SD_D2, pin_d3);
