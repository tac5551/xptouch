...
#define HISTORY_REPRINT_BTN_W 72
#define HISTORY_REPRINT_BTN_H 32
#endif
...
lv_obj_t *ui_historyComponent_create(lv_obj_t *comp_parent)
{
    ...
    for (int i = 0; i < XTOUCH_HISTORY_UI_ROW_SLOTS; i++)
    {
        ...
        /* dsc が古い縦横比のまま残っても枠（正方形）に合わせてスケールする */
        lv_img_set_size_mode(coverImg, LV_IMG_SIZE_MODE_VIRTUAL);
        ...
        lv_obj_set_width(reprintBtn, HISTORY_REPRINT_BTN_W);
        lv_obj_set_height(reprintBtn, HISTORY_REPRINT_BTN_H);
