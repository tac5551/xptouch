...
static void ui_history_reprint_detail_reload_timer_cb(lv_timer_t *t)
{
    ...
    if (xPTouchConfig.currentScreenIndex == 16)
    {
        /* 全画面 loadScreen しない。
...
static void ui_history_reprint_detail_fetch_timer_cb(lv_timer_t *t)
{
    ...
    ui_msg_send(XPTOUCH_HISTORY_REPRINT_DETAIL_FETCH,
                 0, 0);
    lv_timer_del(t);
}
...
void ui_historyReprintScreen_screen_init(void)
{
    ...
    lv_msg_subsribe_obj(XPTOUCH_HISTORY_REPRINT_DETAIL_READY, ui_historyReprintScreen, NULL);
    ...
    if (!xptouch_history_reprint_detail_fetch_done)
    {
