...
def post_build_create_ota_json(version_value):
    ...
    md5, size = calculate_md5_and_size(
        f"../xptouch-bin/ota/xptouch.{version_value}.bin")
    ota = {
        "version": version_value,
        "url": f"http://xperiments.in/xptouch-bin/ota/xptouch.{version_value}.bin",
        "md5": md5,
    }
    ...
    with open("../xptouch-bin/ota/ota.json", "w") as ota_file:
...
def post_build_manifest(version_value):
    ...
    # If you want to save it to a file, you can do:
    with open("../xptouch-bin/webusb/webusb.manifest.json", "w") as webusb_manifest_file:
