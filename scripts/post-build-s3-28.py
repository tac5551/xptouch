...
def post_build_create_ota_json(version_value):
    ...
    with open(bin_channel_path("ota", f"xptouch.{version_value}.json"), "w") as ota_versioned_file:
...
def post_build_copy_ota_fw(version):
    ...
    ota_elf_source = build_output_path("firmware.elf")
    ota_elf_target = bin_channel_path("ota", f"xptouch.{version}.elf")
